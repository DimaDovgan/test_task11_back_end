...
const auditPatient = (patientString, patientZvit) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (let index = 0; index < patientsArr.length; index++) {
        ...
        if (Number.isInteger(parseFloat(patientId))) {
            const auth = yield patient_1.default.findOne({ id: patientId });
        ...
        const patientSendDateBase = yield patient_1.default.create(patientSuccesful);
        console.log("patientSendDateBase", patientSendDateBase);
    }
    return patientZvit;
});
const auditDoctor = (doctorString, doctorZvit) => __awaiter(void 0, void 0, void 0, function* () {
    const doctorArr = doctorString.split("\n");
    for (let index = 0; index < doctorArr.length; index++) {
        const doctor = doctorArr[index];
        const doctorSuccesful = {
            id: null,
            time: null,
            name: null,
            dateBirsday: null,
        ...
        if (Number.isInteger(parseFloat(doctorId))) {
            const auth = yield Doctor_1.default.findOne({ id: doctorId });
            // console.log("auth",auth)
            if (auth) {
                console.log("id duplicates");
        ...
        const patientSendDateBase = yield Doctor_1.default.create(doctorSuccesful);
    }
    return doctorZvit;
});
const auditAppointmens = (AppointmensString, AppointmensZvit) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (let index = 0; index < AppointmensArr.length; index++) {
        ...
        if (idPatient) {
            const auth = yield patient_1.default.findOne({ id: idPatient });
        ...
        // idDoctor
        if (idDoctor) {
            const auth = yield Doctor_1.default.findOne({ id: idDoctor });
        ...
        const apointmentSendDateBase = yield Appointment_1.default.create(AppointmenSuccesful);
        console.log("apointmentSendDateBase", apointmentSendDateBase);
    }
    return AppointmensZvit;
});
const sendForms = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const appointmensZvit = {
        succesful: [],
        wrong: [],
        duplicates: []
    };
    const formBody = req.body;
    const zvitPatients = yield auditPatient(formBody.patients, patientZvit);
    //auditDoctor(formBody.doctors,patientZvit).then(console.log)
    //console.log(await auditDoctor(formBody.doctors,patientZvit))
    let zvitDoctors = yield auditDoctor(formBody.doctors, doctorZvit);
    console.log("ggg", zvitDoctors);
    let zvitAppointmens = yield auditAppointmens(formBody.appointments, appointmensZvit);
